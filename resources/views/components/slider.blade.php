<div class="slider__container">
    <div class="slider__container-outer">
        <div class="slider__container-details">
            <h1>Data<span>Door</span></h1>
            <h2>
                Open the door to smarter real estate decisions</h2>
        </div>
        <div class="slider__container-socials">
            <div class="slider__container-socials"></div>
            <hr class="text-[#269ccc]" />
            <h6>Get Connected</h6>
            <a href="#"><i class="fa-brands text-blue-600 fa-linkedin"></i> </a>
            <a href="#"><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="30" height="30" viewBox="0 0 24 24">
                    <path d="M 2.3671875 3 L 9.4628906 13.140625 L 2.7402344 21 L 5.3808594 21 L 10.644531 14.830078 L 14.960938 21 L 21.871094 21 L 14.449219 10.375 L 20.740234 3 L 18.140625 3 L 13.271484 8.6875 L 9.2988281 3 L 2.3671875 3 z M 6.2070312 5 L 8.2558594 5 L 18.033203 19 L 16.001953 19 L 6.2070312 5 z"></path>
                </svg></a>
            <a href="#"><i class="fa-brands fa-instagram"></i></a>
            <a href="#"><i class="fa-brands fa-facebook-f text-blue-700"></i></a>
        </div>
    </div>
</div>
